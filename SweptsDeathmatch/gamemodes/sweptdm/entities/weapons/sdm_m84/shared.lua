...
SWEP.VM = "models/weapons/v_flashbang.mdl"
SWEP.WM = "models/weapons/w_eq_flashbang.mdl"
...
SWEP.Primary.Ammo             = "M84 Grenades"
...
function SWEP:Initialize()
	...
	if CLIENT then
		...
		if not self.Nade then
			self.Nade = self:createManagedCModel("models/weapons/v_flashbang.mdl", RENDERGROUP_BOTH)
		...
		if not self.Owner:KeyDown(IN_ATTACK) and not self.Owner:KeyDown(IN_ATTACK2) then
			if CT > self.TimeToThrow then
				...
				timer.Simple(0.15, function()
					if IsValid(self) and IsValid(self.Owner) and self.Owner:Alive() then
						if SERVER then
							local nade = ents.Create("fas2_thrown_m84")
				...
				timer.Simple(0.6, function()
					if IsValid(self) and IsValid(self.Owner) and self.Owner:Alive() then
						if self.Owner:GetAmmoCount("M84 Grenades") > 0 then
...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		draw.ShadowText(self.Owner:GetAmmoCount("M84 Grenades") .. "x M84", "FAS2_HUD24", x / 2, y / 2 + 200, Color(255, 255, 255, 255), Color(0, 0, 0, 255), 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end
